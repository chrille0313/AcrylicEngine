...
namespace Acrylic {
	...
	void Renderer2D::DrawQuad(const glm::vec2& position, const glm::vec3& rotation, const glm::vec2& size, const Ref<Texture2D> texture, const glm::vec4& tint, float tilingScale)
	{
		DrawQuad({ position.x, position.y, 0.0f }, rotation, size, texture, tint);
	}

	void Renderer2D::DrawQuad(const glm::vec3& position, const glm::vec3& rotation, const glm::vec2& size, const Ref<Texture2D> texture, const glm::vec4& tint, float tilingScale)
	{
		AC_PROFILE_FUNCTION();
		...
		s_Data->TextureShader->SetFloat("u_TilingScale", tilingScale);
