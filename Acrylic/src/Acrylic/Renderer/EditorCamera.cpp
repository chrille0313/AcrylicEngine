...
namespace Acrylic {
	...
	void EditorCamera::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<MouseScrolledEvent>(AC_BIND_EVENT_FN(EditorCamera::OnMouseScroll));
	}
	...
	void EditorCamera::UpdateProjection()
	{
		...
		m_ViewProjection = glm::perspective(glm::radians(m_FOV), m_AspectRatio, m_NearClip, m_FarClip);
	}
	...
	{
		float xMax = 2.4f, yMax = 2.4f;

		float x = std::min(m_ViewportWidth / 1000.0f, xMax); // max = 2.4f;
		float xFactor = 0.0366f * (x * x) - 0.1778f * x + 0.3021f;

		float y = std::min(m_ViewportHeight / 1000.0f, yMax); // max = 2.4f
